...
from utils.utils import to_torch, flatten_time, get_spaces
...
class PPOAgent():
    ...
    def sample_action(self,
        ...
        with torch.no_grad():
            th_observation = to_torch(self.agent.network.device, observation).float()
...
class PPO():
    ...
    def update(self) -> Dict[str, float]:
        ...
        for epoch in range(self.args.epochs):
            for advantages, returns, action, old_log_prob, *replay_data in self.rollout_loader(*rollout_data):
                ...
                loss = value_loss * self.args.value_coef + policy_loss + entropy_loss * self.args.ent_coef

                self.agent.optimizer.zero_grad()
                loss.backward()
                torch.nn.utils.clip_grad_norm_(self.agent.network.parameters(), self.args.max_grad_norm)
    ...
    def rollout_to_torch(self, sample: np.ndarray) -> Tuple[torch.Tensor]:
        th_obs, th_action, th_next_obs, th_old_log_prob = to_torch(self.agent.network.device, [
            sample["observation"],
            sample["action"],
    ...
    def prepare_rollout(self, sample: np.ndarray) -> List[torch.Tensor]:
        ...
        _, th_flatten_values = self.agent.network(self.flatten_time(th_obs))
        th_values = th_flatten_values.reshape(env_size, rollout_size, 1)
        _, th_next_value = self.agent.network(th_next_obs)

        advantages, returns = to_torch(self.agent.network.device, calculate_gae(
    ...
    def setup(env: VecEnv,
        ...
        observation_space, action_space, action_dim = PPO.get_spaces(env)

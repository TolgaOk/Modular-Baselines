...
class Policy(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.optimizer = torch.optim.Adam(
            itertools.chain(self.model.actor.parameters(),
    ...
    def _forward(self, tensor):
        with torch.no_grad():
            ...
            logits, _, rstate = self.model(processed_tensor)
            features = self.model.make_feature(logits, rstate)

        values = self.model.critic(features)
        actor_logits = self.model.critic(features)
...
def run(args):
    ...
    # Model
    model = JTACModel(vecenv.observation_space.shape[0],
...
if __name__ == "__main__":
    ...
    parser.add_argument("--a2c-lr", type=float, default=7e-4,
                        help="Learning rate")

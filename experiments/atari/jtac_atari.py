...
from modular_baselines.contrib.jacobian_trace.models.vision import (ObservationEncoder,
                                                                    ObservationDecoder)
...
def create_fc_policy(input_shape, action_size, args):
    encoder_model = ObservationEncoder(
        depth=32,
        stride=2,
        shape=input_shape,
        activation=torch.nn.ReLU
    )
    embed_size, latent_shape = encoder_model.embed_size
    embedding_size = embed_size // np.product(latent_shape)

    decoder_model = ObservationDecoder(
        depth=32,
        stride=2,
        activation=torch.nn.ReLU,
    ...
    transition_model = DenseModel(
        input_size=np.product(latent_shape) * args.n_embeddings,
        output_size=np.product(latent_shape) * args.n_embeddings * action_size,
        layers=2,
        hidden_size=200,
        output_shape=(*latent_shape, args.n_embeddings, action_size),
        activation=torch.nn.ELU)
    actor_model = DenseModel(
        input_size=np.product(latent_shape) * embedding_size,
...
def run(args):
    ...
    # Policy
    policy = create_fc_policy(vecenv.observation_space.shape,
                              vecenv.action_space.n,
                              args)

    # Modules
    buffer = JTACBuffer(buffer_size=max(args.rollout_len, args.buffer_size // args.n_envs),
...
class AtariJTACrunner(OptimizerRunner):
    def single_run(self, args: namedtuple):
        return run(args)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Pong JTAC")
    parser.add_argument("--n-envs", type=int, default=16,
                        help="Number of parallel environments")
    parser.add_argument("--seed", type=int, default=None,
                        help="Global seed")
    parser.add_argument("--envname", type=str, default="PongNoFrameskip-v4",
    ...
    parser.add_argument("--tune/actor-lr", type=float, nargs="+", default=[1e-4],
                        help="TUNE: Learning rate")
    parser.add_argument("--tune/critic-lr", type=float, nargs="+", default=[1e-4],
                        help="TUNE: Learning rate")
    parser.add_argument("--tune/model-lr", type=float, nargs="+", default=[1e-3],
                        help="TUNE: Learning rate")
    parser.add_argument("--tune/ent-coef", type=float, nargs="+", default=[0.01],
    ...
    AtariJTACrunner(args, runs_per_job=runs_per_job, seed_repeat=seed_repeat).run(n_jobs=n_jobs)

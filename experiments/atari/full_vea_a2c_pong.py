...
from modular_baselines.contrib.model_based.latent_a2c import FullLatentA2C
from modular_baselines.contrib.model_based.vae import FullLatentVae
...
class FullLatentPolicy(Policy):
    ...
    def _forward(self, tensor):
        ...
        latent = dist.loc if self.use_latent_mean else dist.rsample()
        latent = latent.detach() if self.detach_encoder else latent

        act_logit = self.action_layers(latent)
        values = self.value_layers(latent)
...
def run(args):
    ...
    # Policy
    policy = FullLatentPolicy(vecenv.observation_space,
                              vecenv.action_space,
                              ...
                              use_latent_mean=args.use_latent_mean)
    ...
    collector = OnPolicyCollector(env=vecenv,
                                  ...
                                  device=args.device)
    # Model
    model = FullLatentA2C(policy=policy,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--detach-encoder", action="store_true",
                        help="Deatch encoder layer when fed into value and action layers")
    parser.add_argument("--use-latent-mean", action="store_true",
                        help="Fed action and value with the latent mean instead of sample")

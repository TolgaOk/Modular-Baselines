...
from modular_baselines.algorithms.ppo.vg_ppo import VgPPOArgs, VgPPO
from modular_baselines.algorithms.ppo.torch_vg_agent import TorchVGAgent
...
def vg_ppo_setup(experiment_name: str, env_name: str, config: MujocoTorchConfig, seed: int, device: str):
...
vg_ppo_mujoco_config = MujocoTorchConfig(
    args=VgPPOArgs(
        rollout_len=2048,
        ent_coef=1e-4,
        value_coef=0.5,
...
if __name__ == "__main__":
    parser = ArgumentParser("VgPPO Mujoco")
    add_arguments(parser)
    cli_args = parser.parse_args()
    parallel_run(vg_ppo_setup, vg_ppo_mujoco_config, n_procs=cli_args.n_procs,
